...
class SupersetDashboardConfig(models.Model):
    ...
    url = fields.Char(
        string="URL",
        required=True,
        help="Enter the URL for your app. URL must start with 'http://' or 'https://'",
    )
    ...
    "ir.ui.menu",
    string="Parent Menu",
    help="Optional parent menu. If not set, the menu will be created at the top level or under the default."
)
    ...
    def create(self, vals):
        group_vals = {
            ...
            "category_id": self.env.ref("g2p_ati_embed.access_module").id,
            "implied_ids": [(4, self.env.ref("g2p_ati_embed.group_g2p_ati_embed_user").id)],
        }
        ...
        menu_vals = {
            ...
            # "parent_id": self.env.ref("g2p_ati_embed.menu_g2p_ati_embed_dashboards").id,
        ...
        for record in self:
            ...
            if record.group and record.group.exists():
                if (
                    ...
                    == self.env.ref("g2p_ati_embed.access_module").id
                ):
